...
def get_parser():
    ...
    parser.add_argument(
        "-L", "--lines", type=str, nargs="+", help="File names of lines file(s) to load"
    )
    parser.add_argument(
        "-T", "--tree", type=str, nargs="+", help="File names of tree file(s) to load"
    )
    ...
    parser.add_argument(
        "-C",
        "--console",
        action="store_true",  # Store true makes it False by default
        help="Start a ipython console",
    )
    parser.add_argument("-D", "--demo", action="store_true", help="Load demo images")
    return parser
...
def download_sample_stacks():
    ...
    img_stacks_filenames = [
        os.path.join(tmp_dir, "reference.tiff"),
...
def main(
    im_stack_fnames_to_load=None,
    sparse_points_to_load=None,
    lines_to_load=None,
    trees_to_load=None,
    plugin_to_start=None,
    embed_console=False,
):
    ...
    sigProxies = (
        []
    )  # This list will be populated this with pyqtgraph.SignalProxy objects for linking signals and slots
    for i in range(3):
        thisProxy = pg.SignalProxy(
            tasty.axes2D[i].view.scene().sigMouseMoved,
            rateLimit=30,
            slot=tasty.mouseMoved,
        )
        thisProxy.axisID = i  # this is picked up the mouseMoved slot
        sigProxies.append(thisProxy)

        thisProxy = pg.SignalProxy(
            tasty.axes2D[i].view.getViewBox().mouseClicked,
            rateLimit=30,
            slot=tasty.axisClicked,
        )
        thisProxy.axisID = i  # this is picked up the mouseMoved slot
        sigProxies.append(thisProxy)
...
def run():
    ...
    main(
        im_stack_fnames_to_load=img_stack_fnames_to_load,
        sparse_points_to_load=args.sparse_points,
        lines_to_load=args.lines,
        trees_to_load=args.tree,
        plugin_to_start=args.plugin,
        embed_console=args.console,
    )
