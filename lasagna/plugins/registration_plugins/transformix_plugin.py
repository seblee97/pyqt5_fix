...
class plugin(
    LasagnaPlugin, QWidget, transformix_plugin_UI.Ui_transformix_plugin
    ...
    def chooseStack_slot(self, fname_to_choose=False):
        ...
        preferences.preferenceWriter(
            "lastLoadDir",
            lasagna.utils.path_utils.stripTrailingFileFromPath(fname_to_choose),
        )
    ...
    def run_slot(self):
        ...
        if success:  # Display result if it is available
            ...
            self.commandText_label.setText(
                "Transform FAILED! See %s for details " % path_to_log
            )
    ...
    def checkIfReadyToRun(self):
        ...
        if (
            os.path.exists(self.inputImagePath)
            and os.path.exists(self.transformPath)
            and os.path.exists(self.outputDirPath)
        ):
